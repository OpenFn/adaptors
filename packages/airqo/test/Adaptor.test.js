...
describe('get', () => {
  it('is the same function referenced by http.get', () => {
    expect(http.get).to.equal(get);
  });
  ...
  it('makes an authenticated GET request to an arbitrary path', async () => {
    ...
    const state = { configuration };
    const finalState = await get('devices/metadata/grids')(state);
  ...
  it('passes query options through to the request', async () => {
    ...
    const state = { configuration };
    const finalState = await get('devices/metadata/sites', {
      query: { limit: 5 },
    })(state);
...
describe('post', () => {
  it('is the same function referenced by http.post', () => {
    expect(http.post).to.equal(post);
  });
  ...
  it('makes an authenticated POST request with a body', async () => {
    ...
    const state = { configuration };
    const finalState = await post('devices/some-endpoint', {
      foo: 'bar',
    })(state);
...
describe('request', () => {
  it('is the same function referenced by http.request', () => {
    expect(http.request).to.equal(request);
  });
  ...
  it('makes an authenticated request with an arbitrary method', async () => {
    ...
    const state = { configuration };
    const finalState = await request('GET', 'devices/metadata/cohorts')(
      state
    );
  ...
  it('throws a clear error when method is missing', async () => {
    try {
      await request('', 'devices/metadata/sites')({ configuration });
      expect.fail('Should have thrown an error');
    } catch (error) {
  ...
  it('throws a clear error when path is missing', async () => {
    try {
      await request('GET', '')({ configuration });
      expect.fail('Should have thrown an error');
    } catch (error) {
      expect(error.message).to.equal('path must be a non-empty string.');
    }
