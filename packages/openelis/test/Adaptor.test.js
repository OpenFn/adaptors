...
describe('request', () => {
  it('should return an object of metrics', async () => {
    ...
    const finalState = await request('GET', 'OpenELIS-Global/rest/home-dashboard/metrics')(state);
  ...
  it('should successfully update a patient', async () => {
    ...
    const finalState = await request('POST', 'OpenELIS-Global/rest/PatientManagement', testData.post.createPatient.payload)(state);
...
describe('get', () => {
  it('should return an object of metrics', async () => {
    ...
    const finalState = await get('OpenELIS-Global/rest/home-dashboard/metrics')(state);
  ...
  it('should return all orders ready for validation', async () => {
    ...
    const finalState = await get('openELIS-Global/rest/home-dashboard/ORDERS_READY_FOR_VALIDATION')(state);
...
describe('post', () => {
  it('makes a post request to the right endpoint with options', async () => {
    ...
    const finalState = await post('OpenELIS-Global/rest/LogbookResults')(state);
    expect(finalState.response.statusCode).to.eql(200);
