...
export type Observation_SzLabResult_Props = {
    ...
    status?: string;
    subject?: Reference;
    testingLaboratory?: Reference[];
    text?: Narrative;
    value?: Quantity | CodeableConcept | string | boolean | number | Range | Ratio | SampledData | Period;
    [key: string]: any;
};
...
export default function(props: Partial<Observation_SzLabResult_Props>) {
    ...
    if (!_.isNil(props.authorizer)) {
        ...
        dt.addExtension(
            resource,
            "https://hapifhir.eswatinihie.com/fhir/StructureDefinition/SzAuthorizerExtension",
            src
        );
    }
    ...
    if (!_.isNil(props.testingLaboratory)) {
        ...
        dt.addExtension(
            resource,
            "https://hapifhir.eswatinihie.com/fhir/StructureDefinition/SzTestingLabExtension",
            src
        );
    }
    ...
    if (!_.isNil(props.code)) {
        resource.code = dt.concept(dt.lookupValue(
            "https://hapifhir.eswatinihie.com/fhir/ValueSet/SzTestParameterCodeVS",
            props.code
        ));
