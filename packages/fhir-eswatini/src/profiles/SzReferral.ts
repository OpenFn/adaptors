...
export type ServiceRequest_SzReferral_Props = {
    ...
    recipient?: any[] | boolean[] | string[] | number[] | Address[] | Age[] | Annotation[] | Attachment[] | CodeableConcept[] | Coding[] | ContactPoint[] | Count[] | Distance[] | Duration[] | HumanName[] | Identifier[] | Money[] | Period[] | Quantity[] | Range[] | Ratio[] | Reference[] | SampledData[] | Signature[] | Timing[] | ContactDetail[] | Contributor[] | DataRequirement[] | Expression[] | ParameterDefinition[] | RelatedArtifact[] | TriggerDefinition[] | UsageContext[] | Dosage[] | Meta[];
...
export default function(props: Partial<ServiceRequest_SzReferral_Props>) {
    ...
    if (!_.isNil(props.category)) {
        ...
        resource.category = props.category.map((x) => dt.concept(
            dt.lookupValue("http://hl7.org/fhir/ValueSet/servicerequest-category|4.0.1", x)
        ));
    ...
    if (!_.isNil(props.orderDetail)) {
        ...
        resource.orderDetail = props.orderDetail.map((x) => dt.concept(
            dt.lookupValue("http://hl7.org/fhir/ValueSet/servicerequest-orderdetail|4.0.1", x)
        ));
