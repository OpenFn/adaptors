import { expect } from 'chai';

import { sendNiaData , execute } from '../src/Adaptor.js';

describe('sendNiaData', () => {
  it('upload to NIA for Ghana Card Number', async () => {
    const state = {
      configuration: {},
      data: {
        merchantKey: '7834rtyfh8h834hf7834hf',
        babyData: {
          dateOfBirth : '2024-11-12',
          fatherName:'kwamena Ahoi Jn',
          forenames:'',
          gender:'MALE',
          lightwaveETrackerID:'IJggqTs7hxL',
          motherName:'Ama Yaboah',
          noSiblingsInDelivery:'1',
          placeOfBirth:'Accra',
          surname:'Yaboah',
          timeOfbirth:'10:25am',
          weightAtBirth:'3kg',
          birthCertificateNumber:'2323423423',
          babyPicture:'',

        },
        personVouching: {
          etrackerLightwaveID:'IJggqTs7hxL',
          ghanaCardPIN:'GHS-236723623-7',
          relationToBaby:'Mother',
          relativePhone:'026252672',
          relativePicture:''
        },
      },
    };

    const finalState = await execute(
      sendNiaData(state => state.data)
    )(state);

    console.log(finalState.data);
  });
});
