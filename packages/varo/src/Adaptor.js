...
export function getEmsData(callback = s => s) {
  return async state => {
    const varoMetadata = state.varoMetadata;
    const varoData = state.varoData;
    const varoMetadataPath = state.varoMetadataPath;
    const varoDataPath = state.varoDataPath;

    console.log(varoDataPath);
    ...
    varoData.records.forEach(item => {
      const durations = [item.RELT, item.RTCW];
      ...
      if (emsData.records.length >= 3) {
        emsData.records.shift();
      }

      emsData.records.push(record);
    });

    const nextState = { ...composeNextState(state, emsData) };
