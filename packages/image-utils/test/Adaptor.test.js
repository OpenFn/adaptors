...
import { resizeImage, getExifData } from '../src/Utils.js';
import { toBuf, toBase64Str } from './helpers.js';
...
async function getResizedBuffer(filename) {
  const { buffer } = await resizeImage(toBuf(filename), {
    width: 1200,
    height: 1600,
  });
  return buffer;
...
describe('getExifData', () => {
  it('reads metadata from a JPG file', async () => {
    const src = toBuf('3203x4271.jpg');

    const exifData = await getExifData(src, { mergeOutput: false });

    expect(Object.keys(exifData)).to.deep.equal([
      '0th',
      'Exif',
      'GPS',
      'Interop',
...
describe('resize', () => {
  describe('output dimensions', () => {
    it('resizes to the requested dimensions', async () => {
      ...
      const finalState = await resize(src, { width: 1200, height: 1600 })(
        state,
      );
  ...
  describe('output format', () => {
    it('converts PNG to a valid JPEG buffer', async () => {
      const finalState = await resize(toBase64Str('img-4096x4096.png'), {
        width: 1200,
        height: 1600,
      })(state);
    ...
    it('rejects an SVG input', async () => {
      let threw = false;
      try {
        await resize(toBase64Str('img-svg.svg'), { width: 1200, height: 1600 })(
          state,
        );
      } catch {
        threw = true;
  ...
  describe('EXIF handling', () => {
    it('strips GPS tags from an image that carries GPS EXIF', async () => {
      const srcExif = piexif.load(
        toBuf('img-4032x3024-gps.jpg').toString('binary'),
      );
...
describe('stripMetadata', () => {
  ...
  it('produces a valid JPEG buffer', async () => {
    const finalState = await stripMetadata(toBase64Str('img-3203x4271.jpg'))(
      state,
    );
...
describe('metadata', () => {
  ...
  it('returns correct dimensions for a landscape image', async () => {
    const finalState = await metadata(toBase64Str('img-4032x3024-gps.jpg'))(
      state,
    );
  ...
  it('writes { width, height, orientation, size, exif } to state.data', async () => {
    ...
    expect(finalState.data.exif).to.be.an('object');
  });

  it('returns readable GPS tag names for an image with GPS EXIF', async () => {
    const finalState = await metadata(toBase64Str('img-4032x3024-gps.jpg'))(
      state,
    );

    expect(finalState.data.exif).to.include.any.keys(
      'GPSLatitude',
      'GPSLongitude',
      'GPSLatitudeRef',
      'GPSLongitudeRef',
  ...
  it('does not include sub-IFD pointer tags in exif', async () => {
    const finalState = await metadata(toBase64Str('img-4032x3024-gps.jpg'))(
      state,
    );

    expect(finalState.data.exif).to.not.have.any.keys(
      'ExifTag',
      'GPSTag',
      'InteroperabilityTag',
    );
  ...
  it.skip('returns a readable UserComment without the ASCII prefix', async () => {
    const inputBuffer = await getResizedBuffer('img-3203x4271.jpg');
    const embedded = await embedMetadata(inputBuffer, {
      UserComment: 'patient-id=42',
    })(state);
    const finalState = await metadata(embedded.data.buffer)(state);

    expect(finalState.data.exif.UserComment).to.equal('patient-id=42');
  });
...
describe('buffer passing between steps', () => {
  ...
  it('passes buffer from resize() to stripMetadata() explicitly', async () => {
    const resizeState = await resize(toBase64Str('img-4032x3024-gps.jpg'), {
      width: 1200,
      height: 1600,
    })(state);
