...
import { setMockClient } from '../src/Utils.js';
...
describe('createBirthRecord', () => {
  it('creates a birth record', async () => {
    ...
    expect(finalState.data.api_data.document_number).to.equal('BIRTH-121234-2026-0000002');
  });
});
  ...
  it('retrieves a birth record via GET', async () => {
    ...
    const state = {
      configuration: {
        ...
        baseUrl: 'http://tracker.chimgh.org',
      },
    };

    const finalState = await get('/api/v1/UserManagementService/integrations/registrations/birth/123')(state);
